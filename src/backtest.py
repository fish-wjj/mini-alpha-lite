...
def px(code, date, fund=False):
    api = pro.fund_daily if fund else pro.daily
    df  = safe_query(api, ts_code=code, end_date=date, limit=1, fields="close")
    return None if df.empty else float(df["close"].iat[0])

days = trade_days()
m_start = days[days.dt.day == 1]
print(f"回测区间：{m_start.iloc[0].date()} → {m_start.iloc[-1].date()}")

equity, dates = [1.0], []

for d in tqdm(m_start):
    trd = d.strftime("%Y%m%d")
    risk = is_risk_off(trd)
    ...
    alpha = ranked["ts_code"].tolist()          # ← 不会再报 KeyError

    end = (d + pd.offsets.MonthEnd()).strftime("%Y%m%d")
    if end > latest_trade_date():
        break
    ...
    r_stk  = np.mean([(px(c, end) - px(c, trd)) / px(c, trd) for c in alpha]) if alpha else 0

    ret = cfg["core_ratio"] * r_etf + cfg["bond_ratio"] * r_bond + cfg["alpha_ratio"] * r_stk
