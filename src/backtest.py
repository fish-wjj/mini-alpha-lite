...
月度回测（一次性取价，纯向量运算）
------------------------------------------------
* ETF 价格用 fund_daily，股票用 daily
* 回测窗口：2018-01-01 → 最近交易日
* 资金权重来自 config.yaml
"""
...
# 关键修正：open_days 已是 Series，无需 to_series()
...
for i in tqdm(range(len(rebalance_dates) - 1)):
    ...
    if new_codes:
        ...
        price_all = price_all.reindex(columns=price_all.columns.union(px_new.columns))
        price_all.loc[px_new.index, px_new.columns] = px_new
